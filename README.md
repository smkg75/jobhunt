...
| `/jobhunt:inbox` | how far back to read, such as `36h` or `7d` | one `ack`, `reply`, `stage-booked` or `rejected` line in `job-history.md` § Journal per answer received, the replies in `applied.md`, and what the mail asks for in `state.md` |
