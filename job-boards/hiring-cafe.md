...
under a filter on another: a role open only in one city (`remote:false` on
the employer's Recruitee) is listed "Paris, Île-de-France" because Paris is that employer's
headquarters. Its `datePosted` drifts too — one posting read 2026-07-07 here and 2026-08-18 on the
employer's Ashby board. Every retained line is re-read on the employer's ATS before it is scored, and
...
a role title with a qualifier returned zero in Île-de-France where a neighbouring title alone returned
sixty-four. Drop the qualifier before concluding the board is empty.
